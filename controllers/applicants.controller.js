  ...
  try {
    ...
    if (!jobId || !name || !email || !req.file) {
      return res
        ...
        .json({
          statusCode: 400,
          message: "All fields are required, including the resume file.",
        });
    }
    ...
    if (!job) {
      return res
        .status(404)
        .json({ statusCode: 404, message: "Job not found" });
    }

    // Generate a public URL for the uploaded resume
    const resumeURL = `${req.protocol}://${req.get("host")}/uploads/resumes/${
      req.file.filename
    }`;
...
const getApplicants = async (req, res) => {
  try {
    ...
    const applicants = await Applicant.find()
      .populate("jobId", "position experience location")
    ...
    if (!applicants || applicants.length === 0) {
      return res
        .status(404)
        .json({ statusCode: 404, message: "No applicants found." });
    }
...
const joiningForm = async (req, res) => {
  try {
    const {
      name,
      email,
  ...
  try {
    ...
    if (!applicants || applicants.length === 0) {
      return res.status(404).json({
        statusCode: 404,
        message: "No applicants found with family relation details.",
      });
    }
...
const deleteApplicant = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ statusCode: 200, message: "Applicant deleted successfully." });
  } catch (error) {
    console.error(error);
    return res
      .status(400)
